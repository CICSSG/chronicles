...
import { createLegislativePOST, deleteLegislativePOST, editLegislativePOST } from "@/app/actions";
...
export default function LegislativeOverview({ document }: { document: any }) {
  ...
  function handleEditDocument(name: string) {
    const legislative = document?.legislative || [];
    
    const filtered = legislative.filter((officer: { name: string }) => officer.name == name);
  ...
  return (
    <div className="mx-auto flex w-11/12 flex-col gap-5 text-white/95">
      ...
      <div className="flex grow-0 basis-0 flex-row items-center justify-between">
        <div className="">
          <h1 className="text-4xl font-bold">Legislative Council</h1>
              ...
              <form
                onSubmit={(e) => {
                  ...
                  if (image == "" && base64Image != "") {
                    ...
                    createLegislativePOST(formData);
                    setBase64Image("");
                    setImage("");
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <DialogPanel
              ...
              className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95"
            >
              <form action={deleteLegislativePOST}>
                <input
                  type="text"
                  name="id"
                  className="hidden"
