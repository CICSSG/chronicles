...
type InputField = { platform: string; link: string };

import { useEffect } from "react";

export default function DynamicInputFieldsContactInfo({data} : {data?: any}) {
  ...
  const handleAddInput = () => {
    setInputs([...inputs, { platform: "", link: "" }]);
  };
  ...
  return (
    <div className="container flex flex-col gap-2">
      ...
      <input name="contact_data" type="text" value={JSON.stringify(inputs)} className="invisible sr-only" readOnly />
    </div>
  );
}
